...
pub fn set_authorized_adapters(e: &Env, adapters: &Vec<Address>) {
    e.storage()
        .instance()
        .set(&DataKey::AuthorizedAdapters, adapters);
}

pub fn get_authorized_adapters(e: &Env) -> Vec<Address> {
    e.storage()
        .instance()
        .get(&DataKey::AuthorizedAdapters)
        .unwrap_or(Vec::new(e))
}
...
pub fn set_is_initialized(e: &Env, initialized: bool) {
    e.storage()
        .instance()
        .set(&DataKey::IsInitialized, &initialized);
}
...
pub fn set_balance(e: &Env, asset: &Address, balance: i128) {
    e.storage()
        .instance()
        .set(&DataKey::Balance(asset.clone()), &balance);
}
...
pub fn set_bailout_request(e: &Env, id: u64, request: &BailoutRequest) {
    e.storage()
        .instance()
        .set(&DataKey::BailoutRequest(id), request);
}

pub fn get_bailout_request(e: &Env, id: u64) -> BailoutRequest {
    e.storage()
        .instance()
        .get(&DataKey::BailoutRequest(id))
        .unwrap()
}
