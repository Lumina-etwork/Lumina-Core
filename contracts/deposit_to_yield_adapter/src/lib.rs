...
impl DepositToYieldAdapter {
    ...
    pub fn initialize(
        ...
        yield_treasury: Address,
        insurance_treasury: Address,
    ) {
        ...
        env.storage()
            .instance()
            .set(&AdapterDataKey::VestingContract, &vesting_contract);
        env.storage()
            .instance()
            .set(&AdapterDataKey::YieldTreasury, &yield_treasury);
        env.storage()
            .instance()
            .set(&AdapterDataKey::InsuranceTreasury, &insurance_treasury);
        env.storage()
            .instance()
            .set(&AdapterDataKey::IsPaused, &false);
        env.storage()
            .instance()
            .set(&AdapterDataKey::ProtocolCounter, &0u64);
    }
    ...
    pub fn whitelist_protocol(env: Env, admin: Address, protocol: LendingProtocol) {
        ...
        env.storage()
            .instance()
            .set(&AdapterDataKey::ProtocolCounter, &counter);
    ...
    pub fn delist_protocol(env: Env, admin: Address, protocol_address: Address) {
        ...
        env.storage()
            .instance()
            .remove(&AdapterDataKey::WhitelistedProtocols(
    ...
    pub fn deposit_to_yield(
        ...
        let shares_received =
            Self::deposit_to_protocol(&env, &protocol_address, &asset_address, amount);
    ...
    pub fn claim_yield(
        ...
        let claimed_yield =
            Self::claim_yield_from_protocol(&env, &protocol_address, &asset_address, yield_amount);
        ...
        env.storage()
            .instance()
            .set(&AdapterDataKey::VaultPositions(vault_id), &positions);
    ...
    pub fn withdraw_position(
        ...
        let (principal_withdrawn, yield_withdrawn) =
            Self::withdraw_from_protocol(&env, &protocol_address, &asset_address, position.shares);
    ...
    pub fn set_pause(env: Env, admin: Address, paused: bool) {
        ...
        env.storage()
            .instance()
            .set(&AdapterDataKey::IsPaused, &paused);
    }
    ...
    fn get_whitelisted_protocol(env: &Env, protocol_address: &Address) -> LendingProtocol {
        env.storage()
            .instance()
            .get(&AdapterDataKey::WhitelistedProtocols(
                protocol_address.clone(),
            ))
    ...
    fn add_or_update_position(env: &Env, vault_id: u64, new_position: YieldPosition) {
        ...
        let mut found = false;
        for (i, pos) in positions.iter().enumerate() {
            if pos.protocol_address == new_position.protocol_address
                && pos.asset_address == new_position.asset_address
            {
    ...
    fn update_yield_summary(env: &Env, vault_id: u64, deposited_change: i128, yield_change: i128) {
        ...
        env.storage()
            .instance()
            .set(&AdapterDataKey::YieldSummary(vault_id), &summary);
    }
}
