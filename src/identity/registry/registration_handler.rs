...
pub struct RegistrationHandler {
    collector: RefCell<ConfirmationCollector>,
    storage: RefCell<IdentityStore>,
}

impl RegistrationHandler {
    pub fn new(f: usize) -> Self {
        Self {
            collector: RefCell::new(ConfirmationCollector::new(f)),
            storage: RefCell::new(IdentityStore::default()),
        }
    }
    ...
    pub fn register_confirmation(
        ...
        let outcome = self.collector.borrow_mut().add_confirmation(
            node_id.clone(),
            public_key.clone(),
            epoch,
            replica_id,
        ...
        match outcome {
            ...
            CollectorOutcome::ConflictResolved(resolution) => {
                ...
                storage.mark_conflict_resolution(node_id, epoch);
                storage.commit(
...
mod tests {
    ...
    fn concurrent_registrations_commit_exactly_one_identity() {
        let handler = Arc::new(Mutex::new(RegistrationHandler::new(1)));
        ...
        let committed_key = handler
            .lock()
            .expect("handler lock poisoned")
            .committed_key(&node_id)
            .expect("one key committed");
